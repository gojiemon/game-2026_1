...
export default class Game {
  ...
  private elapsed = 0;
  private score = 0;
  private harvestCount = 0; // apples collected by jump
  private shootTimer = 0;
  private spawnTimer = 0;
  private startTime = 0;
  private lastLane: Lane = 0;
  private currentLane: Lane = 0; // track current lane for sprite facing
  private jumpTimer = 0; // remaining jump time; >0 means jumping
  private assetsLoaded = false;
  private assets?: LoadedAssets;
  private loadingMessage = 'Loading...';
  private sound = new Sound();
  private crateLogged = false; // log crate size debug once
  ...
  private checkCollisions() {
    ...
    for (const ball of this.balls) {
      let hit = false;
      if (jumpProbe && this.bulletHitsBall(jumpProbe as unknown as Bullet, ball)) {
        hit = true;
        this.score += 1;
